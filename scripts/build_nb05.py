"""Generate the leak-aware V2 probability-model notebook."""
...
def main() -> None:
    ...
    notebook.cells = [
        nbf.v4.new_markdown_cell(
            ...
            "Les labels viennent uniquement de ``market_outcomes``. Les plis sont croissants : "
            "un résultat résolu après la fin d'entraînement ne peut ni choisir les joueurs ni "
            "entraîner le modèle."
        ),
        ...
        nbf.v4.new_markdown_cell(
            ...
            "Pour rester strict, chaque observation est prise avant ``resolved_at``. Les marchés "
            "encore ouverts ne créent pas de ligne supervisée."
        ),
        nbf.v4.new_code_cell(
        ...
        nbf.v4.new_markdown_cell(
            ...
            "Les scores sont des règles propres : log-loss et Brier. La ligne de marché est une "
            "baseline probabiliste ; la classe majoritaire est une seconde baseline."
        ),
        nbf.v4.new_code_cell(
            "reports = []\n"
            "for fold in folds:\n"
        ...
        nbf.v4.new_markdown_cell(
            "Un edge positif est une comparaison de probabilités tenue à l'écart, non une preuve "
            "de rentabilité. Toute simulation de PnL doit ensuite modéliser exécution et règlement."
        ),
    ]
    for index, cell in enumerate(notebook.cells):
        cell["id"] = f"05-{index:02d}"
    output = Path(__file__).resolve().parents[1] / "examples" / "05-ml-dataset-and-baseline.ipynb"
